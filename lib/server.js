...
class EggRpcServer extends RpcServer {
  constructor(app) {
    ...
    for (const interfaceName in apiMeta) {
      ...
      for (const clazz in classMaps) {
        assert(
          !classMap[clazz],
          `[egg-rpc-server] ${clazz} duplicate definition`
        );
    ...
    app.ready((err) => {
      if (!err) {
        ...
        this.publish();
        this.logger.info(
          "[egg-rpc#server] publish all rpc services after app ready"
        );
      }
    });
  }
  ...
  load() {
    ...
    for (const name in app.rpcServices) {
      ...
      if (delegate.interfaceName || delegate.namespace) {
        service.interfaceName = delegate.interfaceName
      ...
      if (is.class(delegate)) {
        ...
        for (const key of Object.keys(delegate)) {
          const newFunc = (delegate[key] = app.toAsyncFunction(delegate[key]));
          const tmpFunc = async function (param, headers) {
            const self = this;
            self.logger.info(
              JSON.stringify({
                ...headers,
                service: `${interfaceName}.${key}`,
                param,
              })
            );
            ...
            let result;
            try {
              const data = await newFunc(ctx, param);
              result = { code: 0, data };
            } catch (err) {
              result = {
                code: 1,
  ...
  createContext(req, res) {
    ...
    const ctx = this.app.createContext(
      httpReq,
      new http.ServerResponse(httpReq)
    );
    ctx.params = args;
    return ctx;
  }
  ...
  addService(service, delegate) {
    assert(
      service && delegate,
      "[egg-rpc#server] addService(service, delegate) service & delegate is required"
    );
  ...
  _handleUncaughtError() {
    if (this.selfPublish) {
      this.unPublish();
      this.logger.warn(
        "[egg-rpc#server] unPublish all rpc services for uncaughtException in this process %s",
        process.pid
      );
    } else {
      this.logger.warn(
        "[egg-rpc#server] rpc server is down, cause by uncaughtException in this process %s",
        process.pid
      );
    }
  }
